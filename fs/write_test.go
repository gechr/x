...
func TestAtomicWrite(t *testing.T) {
	...
	require.NoError(t, xfs.AtomicWrite(path, []byte("hello"), 0o600))
...
func TestAtomicWrite_OverwritesExisting(t *testing.T) {
	...
	require.NoError(t, xfs.AtomicWrite(path, []byte("new"), 0o600))
...
func TestAtomicWrite_NoTempLeftBehind(t *testing.T) {
	...
	dir := t.TempDir()
	path := filepath.Join(dir, "config.txt")
	require.NoError(t, xfs.AtomicWrite(path, []byte("x"), 0o600))
...
func TestEnsureDir(t *testing.T) {
	...
	require.NoError(t, xfs.EnsureDir(target, 0o755))
...
func TestCopyFile(t *testing.T) {
	...
	require.NoError(t, xfs.CopyFile(src, dst))
...
func TestCopyFile_MissingSource(t *testing.T) {
	...
	dir := t.TempDir()
	err := xfs.CopyFile(filepath.Join(dir, "missing"), filepath.Join(dir, "dst"))
	require.Error(t, err)
}
...
func TestCopyFile_SameFile(t *testing.T) {
	...
	require.NoError(t, xfs.CopyFile(src, src))
...
func TestCopyFile_SameFileViaHardlink(t *testing.T) {
	...
	require.NoError(t, xfs.CopyFile(src, link))
